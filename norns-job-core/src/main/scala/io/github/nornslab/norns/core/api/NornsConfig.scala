...
object NornsConfig {
  ...
  /**
    * Load a new NornsConfig from the Environment.
    */
  def load(environment: Environment, devSettings: Map[String, AnyRef]): NornsConfig = {
    load(
  ...
  /**
    * Load a new NornsConfig from the Environment.
    */
  def load(environment: Environment): NornsConfig = load(environment, Map.empty[String, String])
  ...
  /**
    ...
    */
  def reference: NornsConfig = NornsConfig(ConfigFactory.defaultReference())
  ...
  def from(data: Map[String, Any]): NornsConfig = {
    ...
    NornsConfig(ConfigFactory.parseMap(toJava(data).asInstanceOf[java.util.Map[String, AnyRef]]))
  }
  ...
  private[api] def configError(
    ...
    // todo 待完善异常信息
    new Exception("todo config error")
  }
...
case class NornsConfig(underlying: Config) {
  ...
  import NornsConfig.{asScalaList, logger}

  private[api] def reportDeprecation(path: String, deprecated: String): Unit = {
  ...
  /**
    ...
    */
  def ++(other: NornsConfig): NornsConfig = {
    NornsConfig(other.underlying.withFallback(underlying))
  }
  ...
  /**
    ...
    */
  def getDeprecatedWithFallback(path: String, deprecated: String, parent: String = ""): NornsConfig = {
  ...
  /**
    ...
    */
  @deprecated("Use get[NornsConfig] with reference config entry", "2.6.0")
  def getConfig(path: String): Option[NornsConfig] = getOptional[NornsConfig](path)
  ...
  def getConfigList(path: String): Option[java.util.List[NornsConfig]] =
    readValue[java.util.List[_ <: Config]](path, underlying.getConfigList(path)).map { configs =>
      configs.asScala.map(NornsConfig(_)).asJava
    }
  ...
  /**
    ...
    */
  @deprecated("Use underlying.getConfigList with reference config entry", "2.6.0")
  def getConfigSeq(path: String): Option[Seq[NornsConfig]] = getConfigList(path).map(asScalaList)
