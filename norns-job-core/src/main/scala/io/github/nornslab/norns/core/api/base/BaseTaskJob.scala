...
/**
  ...
  */
abstract class BaseTaskJob extends Job {

  // 根据实际业务重写构建过程
  val taskBuilder: TaskBuilder = new BaseRefTaskBuilder()

  // 待执行 task
  private[this] lazy val tasks: Seq[Task] = {
    val tss = taskBuilder.buildTaskContexts
    info(s"taskData size [${tss.size}]")
    info(s"build tasks...")
    tss.flatten(taskBuilder.buildTasks(context)(_))
  }
